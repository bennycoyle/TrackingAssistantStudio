 ...
 */
public class MYSPFragment extends Fragment implements OnTouchListener {
    ...
    public static Fragment newInstance(Context context) {
        MYSPFragment f = new MYSPFragment();
        return f;
    }

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        rootView = inflater.inflate(R.layout.my_spmenu, container, false);
    ...
    public void checkFieldValues() {
        ...
        if (calories.getText().toString().equals("") &&
            ...
            restartToChange();
                ...
                satFat.getText().toString().equals("") &&
                sugars.getText().toString().equals("") &&
                protein.getText().toString().equals("54321")) {

            //This is where I will put dev work so that I can issue updates without breaking working code.
            tester = pref.getString(SetPrefActivity.KEY_TEST_EGG_VALUE, "");
            SharedPreferences.Editor editor = pref.edit();
            if (tester.equals("0")) {
                System.out.println("Test is 0, changing to 1");
                editor.putString("testerMenu", "1").apply();
                cf.displayErrorMessage(getActivity(), "Restart the App to ADD the tester menu! :)");
            } else if (tester.equals("1")) {
                System.out.println("Test is 1, changing to 0");
                editor.putString("testerMenu", "0").apply();
                cf.displayErrorMessage(getActivity(), "Restart the App to HIDE the tester menu! :)");
            } else {
                System.out.println("This shouldn't happen");
            }
            restartToChange();
